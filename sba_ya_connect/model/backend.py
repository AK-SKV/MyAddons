...
class wc_backend(models.Model):
    ...
    _description = 'YmkCommerce Backend Configuration'
    ...
    @api.multi
    def update_existing_order(self, Ymk_sale_order, data):
    ...
    def check_existing_order(self, data):
        ...
        for val in data['orders']:
            Ymk_sale_order = self.env['Ymk.sale.order'].search(
                [('Ymk_id', '=', val['id'])])
            if Ymk_sale_order:
                self.update_existing_order(Ymk_sale_order[0], val)
                continue
    ...
    def import_category(self):
        ...
        category_import_batch.delay(
            session, 'Ymk.product.category', backend_id,
            {'from_date': from_date,
    ...
    def import_product(self):
        ...
        product_import_batch.delay(
            session, 'Ymk.product.product', backend_id,
            {'from_date': from_date,
    ...
    def import_customer(self):
        ...
        customer_import_batch.delay(
            session, 'Ymk.res.partner', backend_id,
            {'from_date': from_date,
    ...
    def import_order(self):
        ...
        sale_order_import_batch.delay(
            session, 'Ymk.sale.order', backend_id,
            {'from_date': from_date,
