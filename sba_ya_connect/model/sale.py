...
class Ymk_sale_order_status(models.Model):
    _name = 'Ymk.sale.order.status'
    _description = 'YmkCommerce Sale Order Status'
...
class SaleOrder(models.Model):
    ...
    _inherit = 'sale.order'
    status_id = fields.Many2one('Ymk.sale.order.status',
                                'YmkCommerce Order Status')


class YmkSaleOrder(models.Model):
    _name = 'Ymk.sale.order'
    _inherit = 'Ymk.binding'
    _inherits = {'sale.order': 'openerp_id'}
    _description = 'Ymk Sale Order'
    ...
    openerp_id = fields.Many2one(comodel_name='sale.order',
                                 ...
                                 required=True,
                                 ondelete='cascade')
    Ymk_order_line_ids = fields.One2many(
        comodel_name='Ymk.sale.order.line',
        inverse_name='Ymk_order_id',
        string='Ymk Order Lines'
    )
...
class YmkSaleOrderLine(models.Model):
    _name = 'Ymk.sale.order.line'
    _inherits = {'sale.order.line': 'openerp_id'}

    Ymk_order_id = fields.Many2one(comodel_name='Ymk.sale.order',
                                   string='Ymk Sale Order',
                                   required=True,
                                   ondelete='cascade',
                                   select=True)
    ...
    backend_id = fields.Many2one(
        related='Ymk_order_id.backend_id',
        string='Ymk Backend',
        readonly=True,
        store=True,
        required=False,
    ...
    def create(self, vals):
        Ymk_order_id = vals['Ymk_order_id']
        binding = self.env['Ymk.sale.order'].browse(Ymk_order_id)
        vals['order_id'] = binding.openerp_id.id
        binding = super(YmkSaleOrderLine, self).create(vals)
        return binding
...
class SaleOrderLine(models.Model):
    ...
    Ymk_bind_ids = fields.One2many(
        comodel_name='Ymk.sale.order.line',
        inverse_name='openerp_id',
        string="YmkCommerce Bindings",
    )


@Ymk
class SaleOrderLineImportMapper(ImportMapper):
    _model_name = 'Ymk.sale.order.line'
    ...
    def product_id(self, record):
        binder = self.binder_for('Ymk.product.product')
...
class SaleOrderAdapter(GenericAdapter):
    ...
    def _call(self, method, arguments):
        try:
            ...
            # this is the error in the Ymk API
...
class SaleOrderBatchImporter(DelayedBatchImporter):
    ...
    def update_existing_order(self, Ymk_sale_order, record_id):
...
#
        ...
        for record_id in record_ids:
            Ymk_sale_order = self.env['Ymk.sale.order'].search(
                [('Ymk_id', '=', record_id)])
            if Ymk_sale_order:
                self.update_existing_order(Ymk_sale_order[0], record_id)
            else:
...
@Ymk
class SaleOrderImporter(YmkImporter):
    _model_name = ['Ymk.sale.order']
    ...
    def _clean_Ymk_items(self, resource):
        """
        Method that clean the sale order line given by YmkCommerce before
        importing it
    ...
    def _get_Ymk_data(self):
        """ Return the raw YmkCommerce data for ``self.Ymk_id`` """
        record = super(SaleOrderImporter, self)._get_Ymk_data()
        # sometimes we need to clean Ymk items (ex : configurable
        # product in a sale)
        record = self._clean_Ymk_items(record)
        return record
...
class SaleOrderImportMapper(ImportMapper):
    ...
    children = [('items', 'Ymk_order_line_ids', 'Ymk.sale.order.line'),
                ]
    ...
    def status(self, record):
        if record['order']:
            rec = record['order']
            if rec['status']:
                status_id = self.env['Ymk.sale.order.status'].search(
                    [('name', '=', rec['status'])])
                if status_id:
                    return {'status_id': status_id[0].id}
                else:
                    status_id = self.env['Ymk.sale.order.status'].create({
                        'name': rec['status']
                    })
    ...
    def customer_id(self, record):
        if record['order']:
            rec = record['order']
            binder = self.binder_for('Ymk.res.partner')
            if rec['customer_id']:
...
#                 Ymk_partner_id = self.env['Ymk.res.partner'].create(
#                     partner_dict)
...
def sale_order_import_batch(session, model_name, backend_id, filters=None):
    """ Prepare the import of Sale Order modified on Ymk """
