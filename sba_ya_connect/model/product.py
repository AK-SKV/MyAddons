...
class YmkProductProduct(models.Model):
    _name = 'Ymk.product.product'
    _inherit = 'Ymk.binding'
    _inherits = {'product.product': 'openerp_id'}
    _description = 'Ymk product product'
...
class ProductProduct(models.Model):
    ...
    Ymk_categ_ids = fields.Many2many(
        comodel_name='product.category',
        string='Ymk product category',
    )
...
class ProductProductAdapter(GenericAdapter):
    _model_name = 'Ymk.product.product'
    _Ymk_model = 'products/details'
...
#
        ...
        _logger.info('search for Ymk Products %s returned %s',
                     filters, record_ids)
...
@Ymk
class ProductProductImporter(YmkImporter):
    _model_name = ['Ymk.product.product']
    ...
    def _import_dependencies(self):
        ...
        for Ymk_category_id in record['categories']:
            self._import_dependency(Ymk_category_id,
                                    'Ymk.product.category')
    ...
    def _after_import(self, binding):
        ...
        image_importer.run(self.Ymk_id, binding.id)
        return
...
class ProductImageImporter(Importer):
    ...
    def _get_images(self, storeview_id=None):
        return self.backend_adapter.get_images(self.Ymk_id)
    ...
    def run(self, Ymk_id, binding_id):
        self.Ymk_id = Ymk_id
...
class ProductProductImportMapper(ImportMapper):
    ...
    def is_active(self, record):
        """Check if the product is active in Ymk
        and set active flag in OpenERP
        status == 1 in Ymk means active"""
        if record['product']:
            rec = record['product']
    ...
    def categories(self, record):
        if record['product']:
            rec = record['product']
            Ymk_categories = rec['categories']
            binder = self.binder_for('Ymk.product.category')
            category_ids = []
            main_categ_id = None
            for Ymk_category_id in Ymk_categories:
                cat_id = binder.to_openerp(Ymk_category_id, unwrap=True)
                if cat_id is None:
            ...
            result = {'Ymk_categ_ids': [(6, 0, category_ids)]}
...
def product_import_batch(session, model_name, backend_id, filters=None):
    """ Prepare the import of product modified on Ymk """
    if filters is None:
        filters = {}
