...
class YmkBinder(Binder):

    """ Generic Binder for YmkCommerce """


@Ymk
class YmkModelBinder(YmkBinder):

    """
    Bindings are done directly on the binding model.Ymk.product.category

    Binding models are models called ``Ymk.{normal_model}``,
    like ``Ymk.res.partner`` or ``Ymk.product.product``.
    They are ``_inherits`` of the normal models and contains
    the Ymk ID, the ID of the Ymk Backend and the additional
    fields belonging to the Ymk instance.
    """
    _model_name = [
        'Ymk.res.partner',
    ...
    def to_backend(self, record_id, wrap=False):
        ...
        if wrap:
            ...
            if binding:
                binding.ensure_one()
                return binding.Ymk_id
            else:
                return None
        if not record:
            record = self.model.browse(record_id)
        assert record
        return record.Ymk_id
    ...
    def bind(self, external_id, binding_id):
        ...
        :type binding_id: int
        """
        # the external ID can be 0 on Ymk! Prevent False values
        ...
        # avoid to trigger the export when we modify the `Ymk_id`
    ...
    def unwrap_binding(self, binding_id, browse=False):
        ...
        Example: when called with a ``Ymk.product.product`` id,
    ...
    def unwrap_model(self):
        ...
        Example: when called on a binder for ``Ymk.product.product``,
