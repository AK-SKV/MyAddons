...
def call_to_key(method, arguments):
    """ Used to 'freeze' the method and arguments of a call to YmkCommerce
...
def record(method, arguments, result):
    ...
    during synchronisations. Call it from YmkCRUDAdapter._call
...
class YmkCRUDAdapter(CRUDAdapter):

    """ External Records Adapter for Ymk """
    ...
    def __init__(self, connector_env):
        ...
        """
        super(YmkCRUDAdapter, self).__init__(connector_env)
    ...
    def _call(self, method, arguments):
        try:
            _logger.debug("Start calling Ymkcommerce api %s", method)
            api = API(url=self.Ymk.location,
                      consumer_key=self.Ymk.consumer_key,
                      consumer_secret=self.Ymk.consumer_secret,
                      version='v2')
            if api:
...
class GenericAdapter(YmkCRUDAdapter):
    ...
    def search(self, filters=None):
        ...
        :rtype: list
        """
        return self._call('%s.search' % self._Ymk_model,
    ...
    def read(self, id, attributes=None):
        ...
        if attributes:
            ...
            # compatibility patch on YmkCommerce.
            arguments.append(attributes)
        return self._call('%s/' % self._Ymk_model + str(id), [])

    def search_read(self, filters=None):
        """ Search records according to some criterias
        and returns their information"""
        return self._call('%s.list' % self._Ymk_model, [filters])

    def create(self, data):
        """ Create a record on the external system """
        return self._call('%s.create' % self._Ymk_model, [data])

    def write(self, id, data):
        """ Update records on the external system """
        return self._call('%s.update' % self._Ymk_model,
                          [int(id), data])

    def delete(self, id):
        """ Delete a record on the external system """
        return self._call('%s.delete' % self._Ymk_model, [int(id)])
